...
export CUDA_VISIBLE="0,1,2,3" # SFT和Emb使用的多卡 (逗号分隔)
...
main()
{
    ...
    NPY_FILE_text="${ROOT}/${CATEGORY}.emb-${MODEL_NAME}-text_only.npy"
    NPY_FILE_image="${ROOT}/${CATEGORY}.emb-${MODEL_NAME}-image_only.npy"
    ...
    if [ "$START_STEP" -le 2 ]; then
        ...
        yellow "跳过 [阶段2-1] text2emb抽取文本模态..."
    fi
    if [ "$START_STEP" -le 2 ]; then
        green "[阶段2-2] text2emb抽取图像模态..."
        check_path "$TEXT2EMB_MODEL"
        CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE} python ./rq/text2emb/multimodal_amazon_text2emb.py \
            --dataset ${CATEGORY} \
            --root ${ROOT} \
        ...
        yellow "跳过 [阶段2-1] text2emb抽取图像模态..."
    fi
        # ================= [步骤 3] RQ-VAE 独立训练 =================
    if [ "$START_STEP" -le 3 ]; then
        ...
        green "[阶段3] RQ-VAE 独立训练..."
        
        # 1. 训练文本模型 (例如这里取 2 个码字)
        yellow "=> 3.1 训练【文本】RQ-VAE (2个码字)."
        TEXT_CKPT_DIR="./rq_output/${CATEGORY}/text"
        mkdir -p "${TEXT_CKPT_DIR}"
        python rq/rqvae.py \
          --data_path "${ROOT}/${CATEGORY}.emb-${MODEL_NAME}-text_only.npy" \
          --ckpt_dir "${TEXT_CKPT_DIR}" \
          --num_emb_list 256 256 \
          --lr 1e-3 \
          --epochs 5000 \
          --batch_size 20480
        # 2. 训练图像模型 (例如这里取 1 个码字作为演示)
        yellow "=> 3.2 训练【图像】RQ-VAE (1个码字)"
        IMG_CKPT_DIR="./rq_output/${CATEGORY}/image"
        mkdir -p "${IMG_CKPT_DIR}"
        python rq/rqvae.py \
          --data_path "${ROOT}/${CATEGORY}.emb-${MODEL_NAME}-image_only.npy" \
          --ckpt_dir "${IMG_CKPT_DIR}" \
          --num_emb_list 256 256 \
          --lr 1e-3 \
          --epochs 5000 \
          --batch_size 20480
        green "✓ RQ-VAE 双模态独立训练完成"
    else
    ...
    if [ "$START_STEP" -le 4 ]; then
        ...
        green "[阶段4] 生成各模态索引并进行后融合拼接..."
        yellow "=> 4.1 生成【文本】索引"
        python rq/generate_indices.py \
            --dataset ${CATEGORY} \
            --ckpt_path ./rq_output/${CATEGORY}/text/best_collision_model.pth \
            --output_dir ${POSSESS_DATA_OUTPATH}/${CATEGORY}/text_indices \
            --device cuda:${MONO_CUDA}
        yellow "=> 4.2 生成【图像】索引"
        python rq/generate_indices.py \
            --dataset ${CATEGORY} \
            --ckpt_path ./rq_output/${CATEGORY}/image/best_collision_model.pth \
            --output_dir ${POSSESS_DATA_OUTPATH}/${CATEGORY}/image_indices \
            --device cuda:${MONO_CUDA}
        yellow "=> 4.3 脚本缝合双模态 SID (给 token 增加独立前缀防冲突)"
        # 核心逻辑：给原有类似的 <a_1> 分别加个标识，变成 <txt_a_1> 和 <img_a_1>，然后拼合。
        python -c "
import json
text_path = '${POSSESS_DATA_OUTPATH}/${CATEGORY}/text_indices/${CATEGORY}.index.json'
img_path = '${POSSESS_DATA_OUTPATH}/${CATEGORY}/image_indices/${CATEGORY}.index.json'
out_path = '${POSSESS_DATA_OUTPATH}/${CATEGORY}/${CATEGORY}.index.json'
...
for k in text_data.keys():
    # 替换前缀：<a_1> => <t_a_1> 以及 <a_1> => <i_a_1>
    t_codes = [c.replace('<', '<txt_') for c in text_data[k]]
    i_codes = [c.replace('<', '<img_') for c in img_data.get(k, [])]
    
    # 拼接：图像的 1个特征 + 文本的 2个特征 = 后融合3个特征 (或者视你的顺序要求决定)
    merged[k] = i_codes + t_codes
with open(out_path, 'w', encoding='utf-8') as f:
    json.dump(merged, f)
print(f'[OK] 成功合并图文 SID 至最终 JSON。总物品数: {len(merged)}')
print(f'     示例码字形态: {list(merged.values())[0]}')
"
        green "✓ 全新后融合索引生成完成"
    else
        yellow "跳过 [阶段4] 生成索引"
