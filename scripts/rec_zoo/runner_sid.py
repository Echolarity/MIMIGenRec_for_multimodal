...
def main():
    ...
    print("\n--- 3. Dataset Samples (from Train Set) ---")
    for i in range(min(3, len(train_samples))):
        seq_ids, next_id = train_samples[i]
        # 用 id2sid 把内部 index 转换回原始字符串 id, 如果遇到 PAD (通常是 item_num)，就打印 <PAD>
        history_sids = [id2sid.get(x, "<PAD>") for x in seq_ids]
        target_sid = id2sid.get(next_id, str(next_id))
        
        print(f"  Sample {i+1}:")
        print(f"    Raw sequence IDs : {seq_ids}")
        print(f"    Raw target ID    : {next_id}")
        print(f"    Original SIDs seq: {history_sids}")
        print(f"    Original SID tgt : {target_sid}")
    print("=" * 60)
    # ========================== 增加的数据与参数打印逻辑 END ============================
