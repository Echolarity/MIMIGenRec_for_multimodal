...
def set_seed(seed: int):
    ...
    # 2. Python Hash 随机性开关锁定
    os.environ['PYTHONHASHSEED'] = str(seed)
    # 强制 cuBLAS 报错代替默默使用不确定性算法 (可选项)
    ...
    # 4. Transformers / Huggingface 底层内部状态
    try:
...
def build_trie_from_index(
    ...
    # 修改 evaluate.py 第 74 行：
    for item_str in sorted(list(item_strs)):  # 使用 sorted 锁定顺序
        full_ids = tokenizer(prefix + item_str, add_special_tokens=False).input_ids
        item_ids = full_ids[len(prefix_ids) :]
        sequences.append(prefix_ids + item_ids + [eos_id])
    trie = Trie(sequences)
